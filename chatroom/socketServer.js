...
module.exports = function(httpServer){
    ...
    socketServer.on("connect",function(socket){
        ...
        // 6. 服务器socket监听客户端发过来的消息
        socket.on("message",function(data){
            // 6.1 提取收到的消息的类型
            var type = data.type;
            // 6.2 根据收到的消息类型，作用不同的处理
            switch(type){
                case "101": // 用户登录消息
    ...
    function handleUserLogin(socket,data){
        // 6.3.1 保存用户的昵称到socket对象中
        socket.nickname = data.nickname;

        //6.3.2 将新登录用户信息加入到数组中
        //userList.push({sid:socket.id,nickname:data.nickname});
        userList.push({ sid:socket.id,nickname:data.nickname});
